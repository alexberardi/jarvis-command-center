"""
Gemma3Compressed - Compressed prompt provider for Gemma 3 12B Instruct.

Applies the same compression technique proven in Gemma2Compressed, HermesCompressed,
and Qwen25Compressed: replaces the verbose Tools: section (full param details +
examples) with a compact name + full-description listing. The <tools> JSON block
is passed through unchanged — param descriptions in JSON carry critical schema info.

Uses inline rules (not shared core_rules.py — that degraded Hermes from 93% to
58%, so we follow the same inline approach). The only changes vs
Gemma3MediumUntrained are:
...
class Gemma3Compressed(Gemma3MediumUntrained):
    """
    Compressed prompt provider for Google Gemma 3 12B Instruct.

    Replaces the verbose Tools: section with a compact name + full-description
    listing (no param details or examples). Inline rules are preserved
    (shared core_rules.py degrades Hermes accuracy, so we follow the same
    inline approach here).
    """

    @property
    def name(self) -> str:
        return "Gemma3Compressed"
    ...
    def build_system_prompt(
        ...
        # Gemma 3: <tools> XML block (inherited from parent)
...
"""
        ...
        logger.info(
            "Built Gemma3Compressed system prompt: %d chars, %d tools",
            len(system_prompt),
            len(tools),
        )
