...
2. **Mark defaulted date params as required** when you want explicit values:
   - Server currently treats `resolved_datetimes` as required for `get_calendar_events`
     and `get_sports_scores`, even if the client marks them optional.
3. **Expect JSON schema updates** in tool registrations:
   - Date/datetime parameters will include `format: "date"` or `format: "date-time"`.
4. **Expect retry behavior** if parameters are malformed:
...
## Small Model Mode (3B)
Enable `JARVIS_SMALL_MODEL_MODE=true` to reduce prompt size and retry overhead.

Defaults applied when this flag is enabled (unless explicitly overridden):
- `JARVIS_PROMPT_STYLE=compact`
- `JARVIS_EXAMPLES_PER_COMMAND=0`
- `JARVIS_TOOL_ROUTER_FILTER_MIN_CONFIDENCE=0.8`
- `JARVIS_RELATIVE_DATE_RETRY_MAX=1`
- `JARVIS_INVALID_PARAM_RETRY_MAX=1`
